...
// ----------------------------
  ...
  Node.prototype.addNode = function(node) {
    this.nodeShape = $.extend(true, {}, node.shape); // $.extend(true,{},obj) used to copy object, not reference!
    this.nodeTitle= $.extend(true, {}, node.title);
  };

  Node.prototype.addShard = function(shard) {
    this.shards.push( $.extend(true, {}, shard) );
  };

  Node.prototype.addButton = function(btn) {
  ...
  // -- Button Class --
  // --- Constructor for Shape objects to hold data for all drawn objects. ---
  function cButton(btn) { // Object: {'x': .., 'y': .., 'color': .., 'size': .., 'text': ..}
    this.id = btn.id || undefined;
    this.x = btn.x || 0; // Rect setup
    this.y = btn.y || 0;
    this.h = btn.size + 6 || 1;
    this.fill = btn.color || '#000';

    this.text = btn.text || "None"; // Text setup
    this.textHeight= btn.size || 12;
    this.textFill = '#000';
